...
export class AssistanceService {
  ...
  searchAssistancesByInternName(name: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/intern-name?name=${name}&page=${page}&size=${size}`);
  }

  findAssistancesByArea(areaName: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/area?areaName=${areaName}&page=${page}&size=${size}`);
  }
