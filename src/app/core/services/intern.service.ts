...
export class InternService {
  ...
  getAllInterns(page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<PaginatedInterns>(`${this.apiUrl}?page=${page}&size=${size}`);
  }

  searchInternsByName(name: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/name/${name}?page=${page}&size=${size}`);
  }

  findInternsByArea(area: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/area/${area}?page=${page}&size=${size}`);
  }

  findInternsByUniversity(university: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/university/${university}?page=${page}&size=${size}`);
  }

  findInternsByAreaUniversity(idAreaUniversity: string, page: number = 0, size: number = 10): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/area-university/${idAreaUniversity}?page=${page}&size=${size}`);
  }
